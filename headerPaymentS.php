...
<script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js"></script>
  ...
  <body>
    <header class="min-header">
      ...
      $organization_result = strtolower(str_replace(' ', '', $_GET['session']));
      $organization_search = mysqli_real_escape_string($conn, $organization_result);
      $organization = mysqli_real_escape_string($conn, $_GET['session']);
      $donationA = mysqli_real_escape_string($conn, $_GET['donationamount']);
			$donation = number_format($donationA, 2, '.', ' ');
			$donation = str_replace(" ",",",$donation);

			$donating = mysqli_real_escape_string($conn, $_GET['donation']);
			if ($donating != "Single" || $donating != "Monthly") {
				$donating = "Single";
			}
			if ($_GET['rest'] > 0) {

				$restA = ($donationA/100) * 6;
				$rest = number_format($restA, 2, '.', ' ');
      ...
      if ($queryResult == 1) {
        while($row = mysqli_fetch_array($result)){
          echo '<h1><a href="mainpage.php" class="min-logo">Hadles       <img src="data:image/jpeg;base64,'.base64_encode($row['name'] ).'"class="img-thumnail mini_img" /></a></h1>';
